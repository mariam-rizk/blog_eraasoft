    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('dashboard.posts.store') }}" novalidate>
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="is_publish" name="is_publish" required>
                            <option value="0" {{ old('is_publish') == '0' ? 'selected' : '' }}>False</option>
                            <option value="1" {{ old('is_publish') == '1' ? 'selected' : '' }}>True</option>
                        </select>
                        @error('is_publish')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
